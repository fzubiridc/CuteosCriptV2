...
func _apply() -> void:
	...
	var env := scene.get_node_or_null("Env") as WorldEnvironment
	if env and env.environment:
		var e := env.environment
		e.tonemap_mode = Environment.TONE_MAPPER_REINHARDT  # hue-preserving (Pixi)
		e.tonemap_exposure = LightCfg.get_v("exposure")
		e.glow_enabled = LightCfg.get_v("bloom_on") >= 0.5
		e.glow_intensity = LightCfg.get_v("bloom_intensity")
		e.glow_hdr_threshold = LightCfg.get_v("bloom_threshold")
		e.glow_strength = LightCfg.get_v("bloom_strength")
		e.glow_bloom = 0.05

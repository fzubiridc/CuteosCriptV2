...
- **Estado:** **IMPLEMENTADO base + integrado in-game (2026-06-27).** Hecho: `ElementProfile` (Resource) + componente `SpellFX` (núcleo/aura + estela ribbon + GPUParticles2D + luz) + escena de test `scenes/fx/spell_fx_test.tscn` + texturas placeholder en `assets/fx/particles|projectiles/`. **Integración:** `SpellLibrary` (static) lee `tools/rig/spells.json` → `ElementProfile` por vara; `projectile.gd` lo engancha vía `set_element()` en **modo `external`** (suma estela+partículas SIN pisar el bolt existente), gateado por `Projectile.USE_SPELL_FX`; `player.gd` carga el perfil por vara en `_load_staff_bolt` y lo pasa al disparar. **Sin vara configurada en la tool ✨Hechizos = comportamiento idéntico al actual.** Autoría: `tools/spell_tool.html` (tab Hechizos) → `tools/serve.py` (endpoints `/api/spellcfg`, `/api/spellasset`). Compile-check headless OK (exit 0). El arte lo pone Felipe (ver [[feedback_art_vs_logic]]); lo generado por Claude es placeholder. **Pendiente:** que Felipe configure varas + pruebe; pulido PRO+ (impacto coreografiado F4, cast/muzzle F3, audio F5); incluir `spells.json` en el export `.exe` (hoy en `tools/`, sirve para dev). **Escalas (fix 2026-06-27):** la tab Hechizos controla el bolt vía `sViaje`→`ElementProfile.bolt_scale`→`_bolt_scale_mul` (antes el bolt solo miraba `STAFF_BOLT_SCALE`/rig_sync, que está vacío `{}`) y las partículas vía `sTrail`→`particle_scale` (`spell_fx` lo aplica a `scale_min/max`). Para varas configuradas en Hechizos, `sViaje` **manda** sobre `STAFF_BOLT_SCALE` (no hace falta `rig_sync`). `SpellLibrary.reload()` se llama en `_load_staff_bolt` → editar la tool + **reiniciar la corrida / re-equipar** refresca sin recompilar (el caché estático ya no obliga a reiniciar el juego entero). **WYSIWYG completo (2026-06-27):** TODOS los campos de la tab Hechizos pegan in-game — `sImpacto`→escala explosión, `fpsViaje`/`fpsImpacto`→fps de anim del bolt, `light{color,energy,radius}`→`glow` del proyectil (en `external` la luz la maneja el proyectil, no SpellFX) + color de la luz de impacto, el núcleo **NO** lleva aura (el arte de la vara ya trae su glow → control de aura quitado de la tool). `sImpacto` escala **cualquier** impacto (explosión propia de la vara o el `powerboom` por defecto). Aplicado en `set_element()` + `_impact()` de `projectile.gd`.
